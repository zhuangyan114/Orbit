...
标准库单文件：`orbit_client.py`。不装 pip 包，`import` 即可。协议字段以 [`docs/api/orbit-automation-api.md`](../../docs/api/orbit-automation-api.md) 和 [`docs/api/orbit-automation-openrpc.json`](../../docs/api/orbit-automation-openrpc.json) 为准；本文件只写怎么调这个模块。

Node CLI 见 [`clients/node/SESSION.md`](../node/SESSION.md)。**默认一次握手只打一条命令就 `close()`。** 非必须不要把连接留着空转。只有同一条调试流要连打多步（`start`→断点→`continue`→step/read/write→`stop`），并且中间不会空等超过 10 分钟，才复用同一个 `OrbitClient`。

## 前提

- VS Code 里 Orbit 扩展已开，工作区开了 `orbit.automation.enabled`。
- `orbit.automation.allowedScopes` 必须覆盖你请求的 scope。默认只有 `read`；缺 scope 时 handshake 仍成功，后续 mutation 会失败。
...
sys.path.insert(0, str(Path(r"C:\Users\22690\Desktop\AI\Orbit\clients\python")))
...
`enumerate_instances()` / `select_instance()` 只读 registry，不握手。其余调用握手后执行，**退出必须 `close()`** 释放 10 分钟租约。`close()` 失败不要掩盖命令结果。

多窗口同一 `projectId`：

```python
endpoint = select_instance(instances, instance_id="577b919b-889a-47a6-a727-97980bd567d4")
...
try:
    ...
    client.refresh_session()  # start 常返回 phase=starting，轮询到 halted 后再读目标
...
完整硬件覆盖示例：[`verify_orbit_client_hw.py`](verify_orbit_client_hw.py)。单测（假 HTTP，不碰板子）：

```bash
python -m unittest clients/python/test_orbit_client.py
```
...
client.invoke("orbit.breakpoints.add", {
    "breakpoint": {"source": {"path": r"d:\STM32\project\vet6_led\Core\Src\freertos.c", "line": 406}},
...
client.invoke("orbit.expression.readMany", {"expressions": ["aww", "ass", "g_ram_data"]}, context="target")
client.invoke("orbit.expression.writeMany", {"writes": [{"expression": "g_ram_data", "value": "0x5A5AA5A5"}]}, context="targetMutation")
client.invoke("orbit.memory.read", {"address": "0x20000010", "count": 4}, context="target")  # data 是 Base64
...
- 函数静态变量（如 `last_log_tick`）当全局读会失败，要在命中该函数后再读。
