...
1. In the workspace settings set `orbit.automation.enabled` to `true` and grant the scopes you need (`orbit.automation.allowedScopes`; default is `["read"]`).
2. Reload the window. The Extension Host writes a user-scoped registry pointer and a per-window endpoint under global storage.
3. Enumerate instances, then handshake with an explicit `instanceId` when more than one window shares the `projectId`.
4. Start or attach to the visible Orbit session. Target-bound calls must send the exact `sessionId` and `sessionGeneration`.
5. Prefer the Node one-shot CLI or the Python standard-library client. MCP is an optional adapter of the same `/v1/rpc` and `/v1/events` contract.

```bash
node Releases/clients/node/cli.js instances
node Releases/clients/node/cli.js status --instance <id>
node Releases/clients/node/cli.js start "Orbit: J-Link (No Flash)" --instance <id>
```

```python
from orbit_client import OrbitClient, enumerate_instances, select_instance
instances = enumerate_instances()
endpoint = select_instance(instances, instance_id="...")
client = OrbitClient(endpoint)
client.handshake(client={"name": "quickstart"}, requested_scopes=["read"])
```

Hardware mutation (reset, run, step, write, flash) is not part of a default quick start. See [hardware acceptance](orbit-automation-hardware-acceptance.md).

## Migration from legacy `/rpc`

One release cycle keeps `POST /rpc` with the old `ozone.*` method names. The compatibility adapter calls the new services, adds deprecation metadata, and still requires the bearer token. It does **not** grant handshake-free mutations, skip generation fences, or open a second owner.

| Legacy | v1 |
|---|---|
| `plugin-api-endpoint.json` as the only pointer | user-scoped registry + per-window endpoint directory; legacy pointer is `unique` or `ambiguous` |
| `ozone.status` / `ozone.target.getState` | `orbit.session.list` / `orbit.session.snapshot` |
| `ozone.expr.readMany` / `writeMany` | `orbit.expression.readMany` / `writeMany` |
| `ozone.record.*` | `orbit.record.*` (paged) |
| `ozone.experiment.run` | `orbit.experiment.run` |
| MCP `ozone_*` tool names | `orbit_*`; `ozone-mcp-server.js` is only a launcher |

New clients must use `/v1/rpc`. After the compatibility window, the old envelope will be removed rather than extended.

## Safety and compatibility

Real hardware reset, run, step, breakpoint, RAM write, Flash, and memory-write acceptance requires explicit user authorization. Mock tests prove only protocol and implementation behavior. 1.1.0 authorized live evidence covers J-Link native and CMSIS-DAP HID v1 without flash; J-Link legacy and explicit `orbit.target.flash` are out of this release. `flashBeforeDebug: false` never performs Flash-only actions, while explicit `orbit.target.flash` uses the already-selected owner.

The optional MCP server is only an Automation API v1 client. It must use the Node automation client for discovery, handshake, RPC, and paging; it must not parse endpoint files, invent a JSON-RPC envelope, pick the most recently active window, or open a second target owner. Tool names are `orbit_*`.

`tools/list` always emits a `required` array on every object `inputSchema`. All-optional tools use `required: []`. This is required because some hosts (notably opencode) serialize a missing `required` key as `required: null`, and strict OpenAI-compatible gateways that translate the request to Anthropic reject that payload (`standard_violation /required: got null, want array`; opencode issue 15540). The empty array does not change tool arity.
