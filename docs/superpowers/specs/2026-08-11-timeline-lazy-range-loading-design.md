...
For a visible range `[viewStart, viewEnd]` with width `W`, request `[viewStart - 0.5W, viewEnd + 0.5W]`. Clamp both ends to the extension-side history bounds. In auto-follow mode the upper end therefore stops at the latest sample instead of requesting future time. Include at most one predecessor point per expression so traces remain continuous at the requested lower boundary.
