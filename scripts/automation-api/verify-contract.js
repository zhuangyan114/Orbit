...
const eventsSchemaPath = path.join(repoRoot, 'docs', 'api', 'orbit-automation-events.schema.json');
const acceptancePath = path.join(repoRoot, 'docs', 'api', 'orbit-automation-acceptance-matrix.md');
const wireFixturesPath = path.join(repoRoot, 'clients', 'test-fixtures', 'wire-payloads.json');
const clientSourcePaths = [
  path.join(repoRoot, 'clients', 'node', 'src', 'index.ts'),
  path.join(repoRoot, 'clients', 'node', 'src', 'cli.ts'),
  path.join(repoRoot, 'clients', 'node', 'src', 'session.ts'),
  path.join(repoRoot, 'clients', 'python', 'orbit_client.py'),
  path.join(repoRoot, 'Releases', 'mcp', 'orbit-mcp-server.js'),
];
const clientFacadePaths = new Set([
  path.join(repoRoot, 'clients', 'node', 'src', 'index.ts'),
  path.join(repoRoot, 'clients', 'python', 'orbit_client.py'),
]);
...
let eventsSchema;
try {
  eventsSchema = JSON.parse(fs.readFileSync(eventsSchemaPath, 'utf8'));
} catch (error) {
  fail(`cannot read ${path.relative(repoRoot, eventsSchemaPath)}: ${error.message}`);
}

let wireFixtures;
try {
  wireFixtures = JSON.parse(fs.readFileSync(wireFixturesPath, 'utf8'));
} catch (error) {
  fail(`cannot read ${path.relative(repoRoot, wireFixturesPath)}: ${error.message}`);
}
...
if (eventsSchema) {
  if (eventsSchema.$schema !== 'https://json-schema.org/draft/2020-12/schema') fail('event schema must use JSON Schema 2020-12');
  if (eventsSchema.type !== 'object' || eventsSchema.additionalProperties !== false) fail('event schema must be a closed object');
  const requiredEventFields = ['eventId', 'instanceId', 'projectId', 'timestamp', 'type'];
  for (const field of requiredEventFields) {
    if (!eventsSchema.required?.includes(field)) fail(`event schema must require ${field}`);
  }
  if (eventsSchema.properties?.eventId?.pattern !== '^[0-9]{16,20}$') fail('eventId must be a decimal uint64 string');
  if (eventsSchema.properties?.timestamp?.pattern !== '^[0-9]+$') fail('event timestamp must be a decimal string');
  if (eventsSchema.properties?.sessionGeneration?.minimum !== 0) fail('event sessionGeneration must be nonnegative');
  if (eventsSchema.properties?.type?.pattern !== '^[a-z][A-Za-z0-9]*(\\.[A-Za-z0-9]+)+$') fail('event type must be a namespaced identifier');
  if (eventsSchema.properties?.data?.type !== 'object') fail('event data must be an object');
}

if (wireFixtures && typeof wireFixtures === 'object') {
  const ids = new Set();
  for (const [name, request] of Object.entries(wireFixtures)) {
    if (!request || typeof request !== 'object') {
      fail(`wire fixture ${name} must be an object`);
      continue;
    }
    if (request.jsonrpc !== '2.0') fail(`wire fixture ${name} must use JSON-RPC 2.0`);
    if (!catalog.includes(request.method)) fail(`wire fixture ${name} uses unknown method ${request.method}`);
    if (!request.params || typeof request.params !== 'object' || !request.params.context) fail(`wire fixture ${name} must contain params.context`);
    if (ids.has(request.id)) fail(`wire fixture ${name} reuses request id ${request.id}`);
    ids.add(request.id);
  }
}

for (const sourcePath of clientSourcePaths) {
  let source = '';
  try {
    source = fs.readFileSync(sourcePath, 'utf8');
  } catch (error) {
    fail(`cannot read ${path.relative(repoRoot, sourcePath)}: ${error.message}`);
    continue;
  }
  const methodLiterals = [...source.matchAll(/['"`](orbit\.[A-Za-z0-9.]+)['"`]/g)].map(match => match[1]);
  for (const method of methodLiterals) {
    if (!catalog.includes(method)) fail(`${path.relative(repoRoot, sourcePath)} references unknown method ${method}`);
  }
  if (clientFacadePaths.has(sourcePath)) {
    for (const required of ['orbit.handshake', 'orbit.session.snapshot', 'orbit.operation.get']) {
      if (!source.includes(required)) fail(`${path.relative(repoRoot, sourcePath)} must support ${required}`);
    }
  }
}
