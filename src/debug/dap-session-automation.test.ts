...
describe('stackTrace control-crossing protection', () => {
  it('drops a stackTrace whose read window crossed into control work', async () => {
    const backend = runtimeBackend();
    const session = connectedJlinkSession(backend as unknown as OzoneBackend);
    const messages = collect(session);

    // The gate resolves (the read was queued while halted), but a control
    // operation (continue/step) began before the register reads dispatched:
    // the target may be running now, so the read must bail out empty.
    (session as any).beginTargetReadWhenAvailable = async () => true;
    (session as any).controlInProgress = true;
    ...
    expect((backend as any).calls).not.toContain('getCallStack');
  });
});
