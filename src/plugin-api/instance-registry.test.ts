...
describe('channel and registry pointer helpers', () => {
  ...
  it('honors ORBIT_AUTOMATION_REGISTRY above platform defaults', () => {
    ...
    expect(defaultEndpointDirectory('D:\\x\\r.json')).toBe(path.join('D:\\x', 'endpoints'));
  });
...
describe('InstanceRegistry registry pointer lifecycle', () => {
  ...
  it('keeps the shared pointer while another same-identity window still has a live endpoint', async () => {
    const first = makeRegistry();
    const second = makeRegistry({}, first.memoryFs);
    first.probe.mockImplementation(async (healthUrl: string) => {
      if (healthUrl.includes(':45123')) return { ok: true, instanceId: first.registry.getInstanceId() };
      if (healthUrl.includes(':45124')) return { ok: true, instanceId: second.registry.getInstanceId() };
      return { ok: false };
    });
    second.probe.mockImplementation(first.probe);

    await first.registry.start(boundServer(45123));
    await second.registry.start(boundServer(45124));
    await first.registry.dispose();

    const pointer = readJson<{ registries: Array<Record<string, unknown>> }>(first.memoryFs, POINTER_PATH);
    expect(pointer.registries).toEqual([
      expect.objectContaining({ channel: 'stable', endpointDirectory: ENDPOINT_DIR }),
    ]);
    expect(first.memoryFs.files.has(path.normalize(endpointFile(first, first.registry.getInstanceId())))).toBe(false);
    expect(first.memoryFs.files.has(path.normalize(endpointFile(second, second.registry.getInstanceId())))).toBe(true);
  });
