...
describe('Automation API configuration', () => {
  it('is disabled by default and grants only read scope by default', () => {
    ...
    expect(packageScripts['test:automation-acceptance']).toBe('node scripts/automation-api/acceptance-validator.js');
    expect(packageScripts['test:automation-multi-window']).toBe('node scripts/automation-api/verify-multi-window.js');
    expect(packageScripts['test:automation-hardware']).toBe('node scripts/automation-api/verify-hardware.js --mock');
  });
});
