...
import { SessionRegistry, automationLifecyclePatch } from './session-registry';
...
describe('automationLifecyclePatch', () => {
  it('maps target.stopped to a halted patch preserving the stop reason', () => {
    expect(automationLifecyclePatch('target.stopped', { reason: 'breakpoint' })).toEqual({
      phase: 'halted',
      targetState: 'halted',
      stopReason: 'breakpoint',
    });
    expect(automationLifecyclePatch('target.stopped', { reason: 'entry' })).toEqual({
      phase: 'halted',
      targetState: 'halted',
      stopReason: 'entry',
    });
    expect(automationLifecyclePatch('target.stopped', {})).toEqual({
      phase: 'halted',
      targetState: 'halted',
    });
  });

  it('maps target.running to a running patch clearing stale halt metadata', () => {
    expect(automationLifecyclePatch('target.running', {})).toEqual({
      phase: 'running',
      targetState: 'running',
      stopReason: null,
      pc: null,
  ...
  it('ignores lifecycle types without a registry phase mapping', () => {
    expect(automationLifecyclePatch('target.connectionLost', { reason: 'DeviceRemoved' })).toBeUndefined();
    expect(automationLifecyclePatch('session.started', {})).toBeUndefined();
    expect(automationLifecyclePatch('breakpoints.changed', {})).toBeUndefined();
  });

  it('mirrors a non-API breakpoint stop into the registry phase instead of leaving it running', () => {
    const { registry, events } = setup();
    const s1 = makeSession('s1');
    registry.onStarted(s1);
    registry.update(s1, { phase: 'running', targetState: 'running' });

    const patch = automationLifecyclePatch('target.stopped', { reason: 'breakpoint' });
    expect(patch).toBeDefined();
    registry.update(s1, patch!);
    ...
    expect(events()[2].data).toEqual({ phase: 'halted', previousPhase: 'running' });
  });
});
