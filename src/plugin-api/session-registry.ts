...
/**
 * Maps a sanitized automation lifecycle event (AUTOMATION_LIFECYCLE_EVENT
 * from the DAP adapter) onto the registry patch mirroring the actual target
 * state. Only `target.stopped` / `target.running` carry a phase; every other
 * lifecycle type leaves the registry untouched. Without this mirror the
 * registry phase only advanced on API-initiated control operations, so
 * breakpoint hits and UI-driven stops left `status`/snapshot stale (running
 * while the target was in fact halted).
 */
export function automationLifecyclePatch(type: string, body: unknown): SessionUpdatePatch | undefined {
  if (type === 'target.stopped') {
    const patch: SessionUpdatePatch = { phase: 'halted', targetState: 'halted' };
    const reason = (body as { reason?: unknown } | undefined)?.reason;
    if (typeof reason === 'string') patch.stopReason = reason;
    return patch;
  }
  if (type === 'target.running') {
    // Running clears any stale halted reason/PC.
    return { phase: 'running', targetState: 'running', stopReason: null, pc: null };
  }
  return undefined;
}
